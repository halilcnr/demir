...
export class SokProvider extends BaseProvider {
  retailerSlug = 'sok';
  retailerName = 'ŞOK';
  ...
  protected getStrategies(): ScrapeStrategy[] {
    return [
      ...
      },
      {
        name: 'css-selectors',
        run: (_html, url, $) => {
          // ŞOK Market product page selectors
          const title = $('h1.product-title, h1.product-name, .product-detail-title, [class*="productName"]').first().text().trim()
            || $('h1').first().text().trim();

          const priceText = $('[class*="product-price"] .current, .price-new, .product-price .discounted, [class*="currentPrice"]').first().text().trim()
